...
//       const lastSession = await History.findOne().sort({ sessionid: -1 }).limit(1); 
...
//         auth_token: token,
//         session: { sessionid: sessionId, sessionname: sessionName }, 
//       });
//   } catch (err) {
...
const login = async (req, res) => {
  ...
  const { username, password, initialPrompt } = req.body;
  ...
  try {
    ...
    if (existingSession) {
      ...
      // Dynamically generate session name
      sessionName = initialPrompt
        ? `Chat: ${initialPrompt.slice(0, 30)}...` // Use the first 30 characters of the prompt
        : `Chat on ${new Date().toLocaleString()}`; // Default to a timestamp-based name
